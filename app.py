...
if uploaded_file is None:
    ...
    # Zmieniony profil testowy: wyższe zużycie w dzień, żeby symulować biznes
...
# --- LOGIKA OPŁATY MOCOWEJ (PRAWIDŁOWA KWALIFIKACJA K1-K4) ---
def kwalifikacja_k(szczyt_mocowy, pozaszczyt_mocowy):
    calkowite = szczyt_mocowy + pozaszczyt_mocowy
    if calkowite == 0: return "K1", 0.17
    # Zgodnie z ustawą: Różnica (Szczyt - Pozaszczyt) / Całkowite
...
st.markdown("---")
st.subheader("💡 Wpływ PV na Opłatę Mocową i Kwalifikację (K1-K4)")
...
c3.metric("Zysk na samej opłacie mocowej", f"{(koszt_mocowy_przed - koszt_mocowy_po):,.0f} PLN".replace(',', ' '))

st.info("Algorytm zgodnie z prawem bada relację różnicy zużycia w szczycie (dni robocze 7-22) i poza nim. PV mocno ścina zużycie dzienne, co potrafi zrzucić klienta z kategorii K4 (100% płatne) do K1 (17% płatne).")
