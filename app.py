...
if uploaded_file is not None:
    try:
        # Czytamy surowe bajty, aby obsłużyć różne kodowania
        raw_bytes = uploaded_file.read()
        try:
            # Najpierw próbujemy UTF-8 (standard)
            decoded = raw_bytes.decode('utf-8')
        except UnicodeDecodeError:
            # Jeśli błąd (np. bajt 0xb3), używamy polskiego kodowania Excela
            decoded = raw_bytes.decode('cp1250')
        
        # Wczytujemy dane: sep=None automatycznie wykryje średnik lub przecinek
        # decimal=',' obsłuży polskie przecinki w liczbach (np. 21,18)
        df_raw = pd.read_csv(io.StringIO(decoded), sep=None, engine='python', decimal=',')
        
        # Dynamiczne przypisanie kolumn na podstawie Twojego screena (Data, Godzina, Wartość)
        df_raw.columns = ['Data', 'Godzina', 'Wartość']
        
        # Łączymy datę i godzinę w jeden punkt czasowy
        df_raw['Timestamp'] = pd.to_datetime(df_raw['Data'] + ' ' + df_raw['Godzina'], dayfirst=True)
        df_raw = df_raw.set_index('Timestamp')
        
        # Agregacja 15-min -> 1h (sumujemy 4 pomiary w pełną godzinę)
        df_hourly = df_raw['Wartość'].resample('1H').sum().reset_index()
        df_hourly.columns = ['Data_Czas', 'Pobór']
        
        # Wyciąganie cech czasowych dla stref
        df_hourly['Godzina_Int'] = df_hourly['Data_Czas'].dt.hour
        df_hourly['Roboczy'] = df_hourly['Data_Czas'].dt.weekday < 5
        
        df = df_hourly.head(8760).copy()
        st.success(f"Pomyślnie wczytano i zagregowano dane do {len(df)} godzin.")
    except Exception as e:
        st.error(f"Nie udało się odczytać pliku. Upewnij się, że kolumny to: Data, Godzina, Wartość. Błąd: {e}")
...
if df is None:
    ...
    df['Godzina_Int'] = df['Data_Czas'].dt.hour
    df['Roboczy'] = df['Data_Czas'].dt.weekday < 5

# --- SYMULACJA PV I OBLICZENIA ---
df['Generacja_PV'] = np.maximum(0, np.sin((df['Godzina_Int'] - 6) * np.pi / 12)) 
...
df['Godzina_Mocowa'] = (df['Godzina_Int'] >= 7) & (df['Godzina_Int'] < 22) & df['Roboczy']

def kalkuluj(col_name):
    en = df[col_name].sum() * (cena_mwh / 1000)
...
# --- PREZENTACJA WYNIKÓW ---
st.subheader(f"📊 Zestawienie kosztów: {osd_choice} {taryfa_choice}")
...
# Wykres
st.markdown("---")
avg = df.groupby('Godzina_Int')[['Pobór', 'Nowy_Pobór', 'Generacja_PV']].mean()
fig = go.Figure()
